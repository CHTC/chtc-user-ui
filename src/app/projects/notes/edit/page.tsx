...
function Page() {
  ...
  const handleSubmit = async (note_id: number | null, project_id: number | null, payload: NoteCreate, update: () => void) => {
    try {
...
const NoteFormSuspense = ({note_id, project_id, handleSubmit}: {note_id: number | null, project_id: number | null, handleSubmit: (note_id: number | null, project_id: number | null, payload: NoteCreate, update: () => void) => Promise<void>}) => {
  const {data: project, mutate} = useSWR((note_id && project_id) ? [`/projects/${project_id}/notes/${note_id}`] : null, () => projectFetcher(note_id, project_id), {suspense: true});
...
const NotePage = ({handleSubmit}: {handleSubmit: (note_id: number | null, project_id: number | null, payload: NoteCreate, update: () => void) => Promise<void>}) => {
